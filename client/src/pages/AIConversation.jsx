        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-190px)] lg:h-[650px] w-full max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-2 flex flex-col bg-white dark:bg-bg-card rounded-3xl border-2 border-border shadow-3d-card overflow-hidden h-full">
            ...
            <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4 bg-slate-50/20 dark:bg-slate-950/10">
              {isSimulated && (
                <div className="flex items-start gap-3 px-4 py-3 rounded-2xl border-2 text-xs font-bold bg-amber-500/10 border-amber-500/20 text-amber-700 dark:text-amber-400 dark:bg-amber-950/20 shrink-0">
                  <span className="text-base">🔑</span>
                  <span className="flex-1 text-[11px] leading-tight">No active Grok or Gemini API key found. Operating in local simulated fallback mode. Set GROK_API_KEY or GEMINI_API_KEY inside your server .env for live AI tutor.</span>
                </div>
              )}
            ...
            <div className="border-t-2 border-border p-4 bg-white dark:bg-bg-card relative">
              ...
              {inlineGrammarFeedback && (
                <div className="absolute bottom-24 left-4 right-4 bg-slate-800 text-white p-3.5 rounded-2xl shadow-xl z-20 border border-slate-700 animate-fade-in-up text-xs font-semibold">
              ...
              <div className="flex gap-2.5 items-center">
                ...
                <div className="flex-1 relative flex items-center">
                  <input
                    ...
                    className="w-full pl-4 pr-12 py-3.5 border-2 border-border rounded-2xl text-xs sm:text-sm font-semibold outline-none focus:border-brand-purple bg-bg-main/20 text-text-main"
                  />
                  <Button
                    variant="custom"
                    onClick={checkGrammarOfInput}
                    disabled={!input.trim() || checkingGrammar}
                    className="absolute right-3.5 text-brand-purple hover:scale-110 disabled:opacity-35 transition p-1 hover:bg-brand-purple/5 rounded-lg"
